...
func SetJWTCookie(ctx *gin.Context, userID int) error {
	...
	ctx.SetCookie(config.JWTCookieKeyName, token, 0, "/", "team-3.member0005.track-bootcamp.run", false, true)
	return nil
}
...
func DeleteCookie(ctx *gin.Context) error {
	...
	ctx.SetCookie(config.JWTCookieKeyName, c, -1, "/", "team-3.member0005.track-bootcamp.run", false, true)
	return nil
}
